  ...
  class InternshipApplicationsUpdateTest < ApplicationSystemTestCase
    ...
    test "other employer can see rejection from student confirmation to another employer's internship_offer" do
      travel_to Date.new(2024, 1, 1) do
        ...
        assert_equal %w[canceled_by_student_confirmation approved],
                     InternshipApplication.all.pluck(:aasm_state).sort.reverse

        sign_in(employer_1)
        visit dashboard_candidatures_path
        click_button 'Refusées'
        find 'p.fr-mt-1w.fr-badge.fr-badge--sm.fr-badge--purple-glycine.fr-badge--no-icon',
             text: "L'élève a choisi un autre stage".upcase
      end
    end
