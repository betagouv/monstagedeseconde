...
class StudentDigestAgreementSignedByAllTest < ApplicationSystemTestCase
  ...
  test "scenario D: student signing last does not get agreement_to_sign notification" do
    # Scenario D: Student is the last party to sign the agreement.
    # When student signs (completing all signatures and transitioning to signed_by_all),
    # they should NOT receive an agreement_to_sign notification for that agreement.
    # They should only receive agreement_signed_by_all notification.
    ...
    # Clear the initial agreement_to_sign action created on agreement creation
    MailActionItem.where(
      internship_agreement_id: internship_agreement.id,
      action_name: "agreement_to_sign"
    ).delete_all
    ...
    # Student legal representative signs
    student_legal_rep_signature = create(:signature,
                                         :student_legal_representative,
                                         internship_agreement: internship_agreement,
                                         user_id: student.id)
    ...
    # Now student signs LAST - this should trigger transition to signed_by_all
    clear_enqueued_jobs
    student_signature = create(:signature,
                               :student,
                               internship_agreement: internship_agreement,
    ...
    assert_not_nil agreement_signed_by_all_item,
                  "agreement_signed_by_all should be created when student signs last"

    # Verify that NO agreement_to_sign action exists for this student/agreement combo
    agreement_to_sign_item = MailActionItem.find_by(
      recipient_id: student.id,
      action_name: "agreement_to_sign",
      internship_agreement_id: internship_agreement.id
    )
    assert_nil agreement_to_sign_item,
               "agreement_to_sign should NOT be created for the student (they just signed - no need to notify them to sign)"
    ...
    # Verify email contains agreement_signed_by_all section but NOT agreement_to_sign
    text_body = email.text_part&.body&.to_s || ""
    html_body = email.html_part&.body&.to_s || ""

    assert_match(/Conventions signées par toutes les parties/i, text_body,
                 "Email should contain agreement_signed_by_all section")
    assert_no_match(/Conventions en attente de votre signature/i, text_body,
                    "Email should NOT contain agreement_to_sign section for student who just signed")
  end
end
