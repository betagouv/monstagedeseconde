...
class InternshipOfferPaginationTest < ActiveSupport::TestCase
  setup do
    ...
    @internship_offer_4 = create(:weekly_internship_offer_2nde)
    @internship_offer_4.stats.update_attribute(:total_applications_count, 4)
    @internship_offer_5 = create(:weekly_internship_offer_2nde)
    @internship_offer_5.stats.update_attribute(:total_applications_count, 5)
  end

  test '.next_from (asc)' do
    skip
    from = InternshipOffer.all.joins(:stats)
    assert_equal [1, 2, 3, 4, 5], from.map { |offer| offer.stats.total_applications_count }
  ...
  test '.next_from (desc)' do
    skip
    from = InternshipOffer.all.joins(:stats)
    assert_equal [5, 4, 3, 2, 1], from.map { |offer| offer.stats.total_applications_count }.sort.reverse
    next_item = from.next_from(current: @internship_offer_1,
                               column: :total_applications_count,
                               order: :desc)
                    .limit(1)
  ...
  test '.previous_from (asc)' do
    ...
    assert_equal [1, 2, 3, 4, 5], from.map { |offer| offer.stats.total_applications_count }.sort
