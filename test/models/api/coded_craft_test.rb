...
class CodedCraftTest < ActiveSupport::TestCase
  ...
  test 'autocomplete_by_name find compound craft names' do
    ...
    assert_equal 1, Api::CodedCraft.autocomplete_by_name(term: 'conducteur').size, 'compound with single complete word missed'
    assert_equal 1, Api::CodedCraft.autocomplete_by_name(term: 'trav').size, 'compound halfly spelled with dashes missed'
    assert_equal 1, Api::CodedCraft.autocomplete_by_name(term: 'travaux').size, 'compound fully spelled with dashes missed'
  end

  test 'autocomplete_by_name returns ordered result (by Levenshtein distance on city.name)' do
    # TODO when in production with real data samples
    ...
    # assert_equal coded_craft_2.name, results[2].name
  end
