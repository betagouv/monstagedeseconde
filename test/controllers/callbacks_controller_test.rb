...
class CallbacksControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should persist an educonnect failure event when sign in raises" do
    educonnect_token_stub
    educonnect_userinfo_stub
    educonnect_logout_stub
    stub_sygne_responsible(ine: "1234567890", token: @omogen.token)

    CallbacksController.stub_any_instance(
      :sign_in,
      proc { raise StandardError, "sign in failed for tests" }
    ) do
      assert_difference "EventReport.count", 1 do
        get educonnect_callback_path,
            params: { code: @code, state: @state, nonce: @nonce }
      end
    end

    report = EventReport.last
    assert_equal "educonnect.failure", report.event_name
    assert_equal "educonnect_failure", report.tag
    assert_equal "1234567890", report.student_ine
    assert_equal "0590121L", report.json_payload["school_uai"]
    assert_equal "StandardError", report.json_payload["error_class"]
  end
  ...
  test "should persist an educonnect failure event when logout raises" do
    educonnect_token_stub
    educonnect_userinfo_unknown_stub

    Services::EduconnectConnection.stub_any_instance(
      :logout,
      proc { raise StandardError, "logout failed for tests" }
    ) do
      assert_difference "EventReport.count", 1 do
        get educonnect_callback_path,
            params: { code: @code, state: @state, nonce: @nonce }
    ...
    report = EventReport.last
    assert_equal "educonnect.failure", report.event_name
    assert_equal "callbacks_controller.educonnect.logout", report.stage
    assert_equal "educonnect_failure", report.tag
    assert_equal "0379142G", report.json_payload["school_uai"]
    assert_equal "StandardError", report.json_payload["error_class"]
  end
