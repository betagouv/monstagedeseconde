...
module Dashboard::InternshipOffers
  class UpdateTest < ActionDispatch::IntegrationTest
    ...
    test "PATCH #update as employer owning internship_offer " \
         "fails due to week change with approved applications" do
      travel_to(Date.new(2024, 9, 1)) do
        internship_offer = create(:weekly_internship_offer_3eme)
        create(:weekly_internship_application,
               :approved,
        ...
        # Try to change weeks to different ones
        different_weeks = Week.troisieme_selectable_weeks
                              .last(2)
                              .map(&:id)
        patch(dashboard_internship_offer_path(internship_offer.to_param),
              params: { internship_offer: {
                week_ids: different_weeks
              } })
        error_message = "Semaines de stage : Impossible de changer les semaines de cette offre de stage car" \
