...
class UpdateInternshipAgreementsSchoolRepresentativeTest < ActiveSupport::TestCase
  TASK_NAME = 'support:update_internship_agreements_school_representative'

  Monstage::Application.load_tasks

  def invoke_task(email:, full_name:, school_uai_code:, from_date:)
    Rake::Task[TASK_NAME].reenable
    Rake::Task[TASK_NAME].invoke(email, full_name, school_uai_code, from_date)
  end

  test 'updates school_representative fields on matching agreements' do
    ...
    invoke_task(
      email:           'nouveau@ecole.fr',
      full_name:       'Nouveau_Representant',
      school_uai_code: school.code_uai,
      from_date:       from_date
    )

    agreement.reload
    assert_equal 'nouveau@ecole.fr',    agreement.school_representative_email
    assert_equal 'Nouveau Representant', agreement.school_representative_full_name
  end
  ...
  test 'skips update when email format is invalid' do
    agreement = create(:mono_internship_agreement)
    student   = agreement.internship_application.student
    school    = student.school
    original_email = agreement.school_representative_email
