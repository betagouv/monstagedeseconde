...
class InternshipApplication < ApplicationRecord
  ...
  def selectable_weeks
    ...
    if student.troisieme_or_quatrieme?
      weeks &= student.school.has_weeks_on_current_year? ? student.school.weeks.in_the_future : Week.troisieme_selectable_weeks
    else
  ...
  def employers_filtered_by_notifications_emails
    ...
    emails = potential_employers.map do |potential_employer|
      should_notify?(employer: potential_employer, internship_offer:) ? potential_employer.email : nil
    end
    emails.compact
  end
  ...
  def should_notify?(employer:, internship_offer:)
    AreaNotification.where(notify: true)
