...
class GenerateInternshipAgreement < Prawn::Document
  ...
  def annexe_a
    ...
    @pdf.move_down 5
    @pdf.text "Prénom, nom du tuteur ou de la tutrice de la structure d'accueil si différent du ou de la signataire de la convention :"
  ...
  def signatures
    ...
    headers = [ "Le/la responsable de l'organisme d'accueil\nPrénom, nom (cachet et signature)",
               "Le chef/la cheffe d'établissement\nPrénom, nom (cachet et signature)" ]
    ...
    @pdf.table([ [ @internship_agreement.organisation_representative_full_name.to_s,
                  @internship_agreement.school_representative_full_name.to_s ] ],
               cell_style: { border_width: 0 },
    ...
    @pdf.table([ [ "L'élève majeur ou le/les responsable(s) légal(aux)\nPrénom, nom et signature",
    ...
    student_and_legal_representative_lines = []
    if @internship_agreement.signed_by_student?
      student_and_legal_representative_lines <<
        "#{@internship_agreement.student.presenter.full_name} a signé électroniquement le : "\
        "#{@internship_agreement.student_signature.signature_date.strftime('%d/%m/%Y à %Hh%M')}"
    end
    if @internship_agreement.signed_by_legal_representative?
      ...
      student_and_legal_representative_lines <<
        "#{signator_full_name} a signé électroniquement le : "\
        "#{@internship_agreement.student_legal_representative_signature.signature_date.strftime('%d/%m/%Y à %Hh%M')}"
    end
    left_cell = student_and_legal_representative_lines.join("\n")
    right_cell = referent_paper_mention
    if left_cell.present? || right_cell.present?
      @pdf.table([ [ left_cell, right_cell ] ],
                 cell_style: { border_width: 0 },
                 column_widths: column_widths)
    end
  end
