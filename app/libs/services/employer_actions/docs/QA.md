...
<tr>
<th>Navigation</th>
<th>Ouvert dans un autre onglet</th>
</tr>
</thead>
<tbody>
<tr>
...
- le niveau "urgence faible" (low) embarque avec lui toutes les notifications de niveau supérieur (moyenne forte et critique)
- le niveau "urgence moyenne" embarque avec lui toutes les notifications de niveau supérieur (forte, critique)
- le niveau "urgence forte" embarque avec lui toutes les notifications de niveau supérieur (critique)
- le niveau "urgence critique" ne cible que les notifications d’urgence critique (aujourd’hui sans application)
...
avec différents rôles afin d’éviter à avoir à se déconnecter et se reconnecter sans
cesse pour changer de rôle.
...
2. **Admin** Déclencher l’email récapitulatif d’urgence moyenne (medium) dans interface B.
...
4. **Admin** : Déclencher l’email récapitulatif d’urgence moyenne (medium) (interface B).
...
6. **Admin**  : Re-déclencher l’email récapitulatif d’urgence moyenne (medium) (interface B).
...
4. **Admin** : Déclencher le mail récapitulatif de niveau moyen.
...
3. **Admin** : Déclencher le mail récapitulatif de niveau moyen.
...
2. **Admin** : Déclencher le mail récapitulatif de niveau fort .
...
2. **Admin** : Déclencher le mail récapitulatif de niveau moyen, vérifier le mail "convention à compléter".
3. Compléter la convention, la faire signer par une première partie (élève ou
   responsable légal).
4. **Admin** : Déclencher le mail récapitulatif de niveau moyen, vérifier le mail "convention à signer" /
   "signée par une autre partie" reçu par l'employeur.
5. **Employeur** : signer la convention à son tour (dernière signature
   manquante).
6. **Admin** : Déclencher le mail récapitulatif de niveau moyen : aucun email ne doit être envoyé.
...
2. **Admin** : Déclencher le mail récapitulatif de niveau moyen.
...
2. **Admin** : Déclencher les quatre digests (critical, high, medium, low).
...
2. **Admin** : Déclencher le mail récapitulatif de niveau bas.
...
### L'interface technique MailActionItem [interface A]
...
prévu de cette urgence là. Cette variable est fixée en dur dans le fichier “mail_action_configurable.rb”

**Date d’expiration** : il y a une date de péremption pour chaque action qui est de 40 jours par
défaut ou la date de début de stage moins 2 jours ; **on filtre les mail_action_items dont les 
informations sont périmées**, autrement dit, on n'enverra rien à travers les emails récapitulatifs pour 
les événements périmés.
...
envoyée. Cette variable est fixée dans un fichier “mail_action_configurable.rb”
