...
class CallbacksController < ApplicationController
  ...
  def educonnect
    ...
    if user_info["FrEduCtEleveUAI"].blank?
      handle_educonnect_logout(educonnect, user_info: user_info)
    ...
    begin
      ...
      notify_educonnect_failure(
        stage: "callbacks_controller.educonnect.sign_in",
        exception: e,
        user_info: user_info,
        code_line: __LINE__.to_s
      )
  ...
  def handle_educonnect_logout(educonnect, user_info: nil)
    educonnect.logout
  rescue StandardError => e
    Rails.logger.error("Failed to logout from Educonnect: #{e.message}")
    notify_educonnect_failure(
      stage: "callbacks_controller.educonnect.logout",
      exception: e,
      user_info: user_info,
      code_line: __LINE__.to_s
    )
  ...
  def notify_educonnect_failure(stage:, exception:, user_info:, code_line:)
    payload = {
      stage: stage,
      error_class: exception.class.to_s,
      message: exception.message,
      request_id: request.request_id,
      student_ine: user_info&.dig("FrEduCtEleveINE"),
      school_uai: user_info&.dig("FrEduCtEleveUAI"),
    ...
    Rails.event.tagged("educonnect") do
      Rails.event.tagged("educonnect_failure") do
        Rails.event.notify("educonnect.failure", payload)
      end
    end
  end
