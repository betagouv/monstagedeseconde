...
namespace :retrofit do
  ...
  desc '09/05/2025 - update offers from public offers without group_id'
  ...
  desc '15/05/2025 - update offers from public offers without group_id'
  task fix_offers_groups: :environment do |task|
    # id;secteur;public;public_employer_type
    PrettyConsole.announce_task(task) do
      if Rails.env.production? || Rails.env.development?
        counter = 0
        resource_file_location = 'db/data_imports/2025-05-15_group_updates.csv'
        CSV.foreach(resource_file_location, 'r', headers: true, header_converters: :symbol, col_sep: ';').each do |row|
          id = row[:id].to_i
          offer = InternshipOffer.find_by(id: id)
          next if offer.nil?
