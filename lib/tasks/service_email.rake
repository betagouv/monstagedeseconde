...
namespace :incident_2022_04_06 do
  ...
  task applications_retrofit: :environment do
    created_ones = InternshipApplications::WeeklyFramed.where('created_at >= ?', Date.new(2022,,6).where('created_at <= ?', DateTime.new(2022,4,3,3,00))whre(aasm_state: 'submitted')
    updated_ones = InternshipApplications::WeeklyFramed.where('updated_at >= ?', Date.new(2022,,6).where('created_at <= ?', DateTime.new(2022,4,3,3,00))whre(aasm_state: 'submitted')
  ...
  task users_retrofit: :environment do
    created_ones = User.where('created_at >= ?', Date.new(2022,,6).where('created_at <= ?', DateTime.new(2022,4,3,3,00))whre(confirmed_at: nil).where(phone_token: nil)
